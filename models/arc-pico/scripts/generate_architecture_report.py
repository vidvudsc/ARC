...
OUT_PDF = OUT_DIR / "arc_pico_vl_architecture_report.pdf"
...
def page(pdf, title, subtitle=None):
    ...
    ax.text(0.06, 0.03, "Arc-Pico-VL v0.1 architecture report", fontsize=8, color=COLORS["muted"], transform=ax.transAxes)
    return fig, ax
...
def main() -> None:
    ...
    with PdfPages(OUT_PDF) as pdf:
        fig, ax = page(pdf, cfg.model_name, "15M-class cheap multimodal pretraining testbed")
        cards = [
        ...
        ax.text(
            0.08,
            0.42,
            "Arc-Pico is intentionally tiny. It is meant to prove tokenizer, data, mixed text/image training, checkpointing, and image-use validation before Arc-124M spends serious GPU money.",
            fontsize=13,
            color=COLORS["ink"],
            wrap=True,
            transform=ax.transAxes,
        ...
        ax.text(0.58, 0.64, "500M mixed tokens/equivalent\n85% text-only\n15% image-caption\nsingle GPU\nunder about 5 EUR target", fontsize=12, transform=ax.transAxes)
        pdf.savefig(fig, bbox_inches="tight")
        plt.close(fig)

    meta = {"pdf": str(OUT_PDF), "model": cfg.model_name, "params": params, "pages": 3}
    (OUT_DIR / "arc_pico_vl_architecture_report.summary.json").write_text(json.dumps(meta, indent=2))
