...
def rotate_step_checkpoints(out_dir: str | Path, keep: int) -> None:
    if keep < 0:
        return
    root = Path(out_dir)
    checkpoints = sorted(root.glob("arc_pico_mixed_step_*.pt"), key=lambda path: path.stat().st_mtime, reverse=True)
...
def main() -> None:
    ...
    for step in range(start_step, args.max_steps):
        ...
        if is_main_process(rank) and (step + 1) % args.save_every == 0:
            save_mixed_checkpoint(
                Path(args.out_dir) / f"arc_pico_mixed_step_{step + 1}.pt",
                model=model,
                optimizer=optimizer,
                scheduler=scheduler,
                cfg=cfg,
    ...
    if is_main_process(rank):
        save_mixed_checkpoint(
            Path(args.out_dir) / "arc_pico_mixed_last.pt",
            model=model,
            optimizer=optimizer,
            scheduler=scheduler,
            cfg=cfg,
