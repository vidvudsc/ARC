...
from .data_vision import DEFAULT_CAPTION_PREFIXES, ImageCaptionCollator, ImageCaptionDataset, load_vision_filter, load_vl_plan
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--checkpoint", default="checkpoints/arc_pico_mixed_last.pt")
...
def rotate_step_checkpoints(out_dir: str | Path, phase: str, keep: int) -> None:
    ...
    root = Path(out_dir)
    checkpoints = sorted(
        root.glob(f"arc_pico_vl_{phase}_step_*.pt"),
...
def main() -> None:
    ...
    if not args.dry_run:
        ...
        image_loader = DataLoader(
            ...
            drop_last=True,
            collate_fn=ImageCaptionCollator(tokenizer.pad_id),
        )
        ...
        for step in range(start_step, args.max_steps):
            ...
            if is_main_process(rank) and (step + 1) % args.save_every == 0:
                path = Path(args.out_dir) / f"arc_pico_vl_{args.phase}_step_{step + 1}.pt"
                save_checkpoint(
                    path,
                    checkpoint_payload(
        ...
        if is_main_process(rank):
            save_checkpoint(
                Path(args.out_dir) / f"arc_pico_vl_{args.phase}_last.pt",
                checkpoint_payload(
                    model=unwrap_model(model),
                    optimizer=optimizer,
                    scheduler=None,
