...
def caption_values(row: dict) -> list[str]:
    for key in ("caption", "captions", "alt_text", "original_alt_text", "sentence", "sentences"):
        value = row.get(key)
        ...
        if isinstance(value, list):
            ...
            for item in value:
                if isinstance(item, str) and item.strip():
                    ...
                    text = item.get("raw") or item.get("caption") or item.get("text") or item.get("sentence")
...
def image_ref(row: dict, *, image_out_dir: Path | None = None, index: int = 0) -> str | None:
    for key in ("image", "image_path", "path", "file_name", "filename", "url"):
        ...
        if image_out_dir is not None and isinstance(value, Image.Image):
            ...
            path = image_out_dir / f"flickr30k_{index:09d}.jpg"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Prepare Flickr30k-style captions as Arc image-caption JSONL")
    parser.add_argument("--source_jsonl", default=None)
    parser.add_argument("--dataset", default="AnyModal/flickr30k")
    parser.add_argument("--config", default=None)
    parser.add_argument("--split", default="train")
    parser.add_argument("--out", default="data/vl/flickr30k_captions.jsonl")
    parser.add_argument("--image_out_dir", default=None, help="Optional directory for datasets that yield PIL images")
    parser.add_argument("--max_examples", type=int, default=150_000)
        ...
        for row in tqdm(rows, total=args.max_examples if args.source_jsonl else None, dynamic_ncols=True):
            ...
            for caption in captions:
                ...
                if args.dedup and pair in seen_pairs:
                    ...
                    continue
                seen_pairs.add(pair)
                handle.write(json.dumps({"source": "flickr30k", "image": image, "caption": caption}) + "\n")
                kept += 1
